...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-6 flex items-center justify-between">
        ...
        <div class="flex gap-2">
            <button id="create-question-btn" type="button" class="px-3 py-2 bg-green-600 text-white rounded hover:bg-green-700">Tạo câu hỏi</button>
        </div>
    </div>
        ...
        <form method="get" class="mb-4 flex flex-wrap gap-2 items-end" id="filterForm">
            ...
            <div>
                ...
                <select name="reading_set_id" id="filter_set_id" class="border rounded p-1 min-w-[120px]">
                    <option value="">-- All --</option>
                    @foreach($sets as $set)
                        <option value="{{ $set->id }}" data-quiz="{{ $set->quiz_id }}" data-part="{{ $set->quiz->part ?? '' }}" data-skill="{{ $set->quiz->skill ?? '' }}" @if(request('reading_set_id') == $set->id) selected @endif>
                            {{ $set->title }}
                        </option>
                    @endforeach
                </select>
