        ...
        <div class="p-3 border rounded mb-3">
            <div class="mb-2 flex items-start gap-3">
                ...
                <div class="flex-1 relative">
                    ...
                    <input type="hidden" name="part4_choice[{{ $i }}]"
                        value="{{ isset($selected[$i]) ? e($selected[$i]) : '' }}" class="part4-hidden-{{ $i }}">
                    ...
                    @php
                        ...
                        if (isset($selected[$i])) {
                            foreach ($pairs as $pair) {
                                if ((string) $pair['idx'] === (string) $selected[$i]) {
                                    $currentLabel = $pair['label'];
                                    break;
                                }
                            }
                        }
                    ...
                    <button type="button"
                        class="w-full bg-white border border-gray-200 rounded-lg p-2 text-sm text-left flex items-center justify-between custom-select-button"
                        data-select-index="{{ $i }}">
                    ...
                    <div
                        class="custom-select-panel hidden absolute left-0 right-0 mt-2 bg-white border border-gray-200 rounded shadow-lg z-50 max-h-56 overflow-auto text-sm leading-normal">
                        <div class="px-3 py-1 hover:bg-blue-600 hover:text-white cursor-pointer select-option text-sm"
                            data-value="">- Select -</div>
                        @foreach($pairs as $pair)
                            <div class="px-3 py-1 hover:bg-blue-600 hover:text-white cursor-pointer select-option whitespace-normal break-words text-sm"
                                data-value="{{ e($pair['idx']) }}">{!! $pair['label'] !!}</div>
                        @endforeach
                    </div>
                </div>
            </div>
            @php
                ...
                if (is_string($val)) {
                    $s = $val;
                } elseif (is_scalar($val)) {             // int, float, bool
                    $s = (string) $val;
                } elseif (is_object($val) && method_exists($val, '__toString')) {
                    $s = (string) $val;
                } else {                                  // array hoặc object không __toString
                    $s = json_encode($val, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
                }
            @endphp

            <div class="mt-3 text-sm text-gray-700">{!! nl2br(e($s)) !!}</div>
        </div>
    @endforeach
