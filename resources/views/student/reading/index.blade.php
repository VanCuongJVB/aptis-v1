...
<div class="container mx-auto py-6 px-6">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold">Chọn bộ đề Reading</h1>
        <p class="text-sm text-gray-500">{{ $sets->count() }} bộ đề công khai</p>
    </div>

    @if($sets->isEmpty())
        <div class="p-6 bg-white border rounded shadow-sm text-center text-gray-600">Hiện chưa có bộ đề công khai cho Reading.</div>
    @else
            ...
            @foreach($sets as $set)
                <div class="bg-white border rounded-lg shadow-sm overflow-hidden flex flex-col">
                    ...
                    <div class="p-4 border-t bg-gray-50 flex items-center justify-between">
                        <a href="{{ route('reading.quiz.start', ['quiz' => $set->quiz_id, 'set_id' => $set->id]) }}" class="btn-base btn-primary px-4 py-2">Làm bài</a>
                        {{-- <a href="{{ route('student.reading.sets.show', $set->id) }}" class="text-sm text-gray-700 hover:underline">Xem bộ đề</a> --}}
                    </div>
                </div>
            @endforeach
        </div>
