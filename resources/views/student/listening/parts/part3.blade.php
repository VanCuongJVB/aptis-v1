...
function submitListeningAnswerPart(questionId) {
	...
	try {
		...
		} else if (meta.hasOwnProperty('answers')) {
			// for matching type where answers contain mapped values, treat selected index mapping
			const arr = Array.isArray(meta.answers) ? meta.answers.map(v => isNaN(v) ? v : Number(v)) : [meta.answers];
			correct = arr;
			// selected may be an index representing a chosen person; assume match by index
